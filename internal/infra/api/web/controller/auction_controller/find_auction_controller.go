...
func (u *AuctionController) FindAuctions(c *gin.Context) {
	...
	if status == "" {
		// Se não for informado, assume valor default Active (0)
		statusNumber = 0
	} else {
		var errConv error
	...
	auctions, err := u.auctionUseCase.FindAuctions(c, auction_usecase.AuctionStatus(statusNumber), category, productName)
	if err != nil {
		errRest := rest_err.ConvertError(err)
		c.JSON(errRest.Code, errRest)
		return
